...
Cập nhật: **2026-09-04**. Đây là điểm bắt đầu để chọn đúng tài liệu và tránh dùng
...
| Làm việc với database | [`DATABASE_SCHEMA.md`](DATABASE_SCHEMA.md), [`SCHEMA_LOGIC.md`](SCHEMA_LOGIC.md) |
...
- `ARCHITECTURE.md`: entry point, bootstrap, context property và ranh giới package.
- `ARCHITECTURE_RULES.md`: quy tắc phụ thuộc bắt buộc.
- `SCHEMA_LOGIC.md`: lifecycle trạng thái và quy ước ghi database.
- `features/*/README.md`: phạm vi, owner, dữ liệu, luồng và giới hạn của từng
  feature.
- `infrastructure/*/README.md`, `core/README.md`, `UI/README.md`:
  contract theo layer.
- `tests/README.md`: cách chạy và cách hiểu kết quả test.
...
- `plan/`: chỉ giữ kế hoạch còn mở; kế hoạch lịch sử đã được loại bỏ để tránh
  nhầm với trạng thái hiện tại.
- `research/`: nguồn báo cáo và sách Typst, không thuộc runtime.

### Mã kế thừa và bên thứ ba

- `archive/backend/README.md`: ranh giới, cách nghiên cứu và cảnh báo của subsystem cũ.
- `vendor/README.md`: provenance, build và cập nhật mã vendored.
- Markdown bên trong `vendor/alacritty/` thuộc upstream Alacritty. Không dùng
  chúng làm tài liệu CAMS và không viết lại trừ khi cập nhật snapshot
  upstream/fork có chủ đích.
- `qtpyTerminal-main/README.md` mô tả adapter tương thích cũ, không phải
