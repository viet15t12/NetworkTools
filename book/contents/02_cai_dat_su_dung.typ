...
- Python `3.11` trở lên;
- `uv` để tạo môi trường Python và cài dependency từ `pyproject.toml` cùng `uv.lock`;
...
Nếu nhận mã nguồn dưới dạng file nén thay vì Git, hãy giải nén toàn bộ repository, mở Terminal tại thư mục `CAMS`, rồi tiếp tục với bước kiểm tra môi trường. Bản mã nguồn phải giữ nguyên các thư mục `UI/`, `core/`, `features/`, `infrastructure/` và file khóa dependency.
...
Sau đó mở *CAMS* từ menu ứng dụng hoặc chạy lệnh `cams` ở bất kỳ thư mục nào.
Bộ cài không cần `sudo`, và việc chạy lại bộ cài để cập nhật không xóa database
người dùng.
...
Đây là vị trí dữ liệu của bản cài Linux. Khi chạy trực tiếp từ mã nguồn, vị trí
mặc định là `CAMS/data/`. Thư mục này chứa database mặc định và trạng thái ứng
dụng như Recent Projects. File project `.ntp` nằm tại vị trí người dùng chọn
trong Create/Open workflow.
...
Kiểm tra quyền ghi đối với thư mục dữ liệu ở trên hoặc thư mục do `CAMS_DATA_DIR` chỉ định. Không chạy nhiều instance cùng ghi một Workspace. Không xóa file `-wal`, `-shm` hoặc journal khi ứng dụng còn chạy.
